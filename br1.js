...
function loginSuccess(user){
  ...
  if(orderNav){
    orderNav.style.display = 
      (currentUser.role==='admin'
      ||currentUser.role==='staff')?'':'none';
  }
...
function processBrCsv(file){
  parseFile(file, function(allRows){
    var si = 0;
    if(allRows.length){
      var fl = String(allRows[0][0]||'').toLowerCase();
    ...
    for(var i=si;i<allRows.length;i++){
      ...
      rows.push({
        ...
        province:String(cols[1]||'').trim(),
        zone:String(cols[2]||'').trim()
      });
    }
    if(!rows.length){
  ...
  parseFile(file, function(allRows){
    ...
    for(var i=si;i<allRows.length;i++){
      ...
      rows.push({
        ...
        desc:String(cols[1]||'').trim(),
        unit:String(cols[2]||'').trim(),
        price:price,
        type:String(cols[4]||'').trim(),
        inuse:inuse
      });
    }
    if(!rows.length){
...
function matSearch(rowId,inp){
  ...
  dd.appendChild(hdr);
  filtered.slice(0).forEach(function(m){
    var item = document.createElement('div');
    item.style.cssText =
...
function chooseCamera(){
  var modal = document.getElementById('photoChoiceModal');
  var g = modal.getAttribute('data-group');
  var i = parseInt(modal.getAttribute('data-index'));
  closePhotoChoice();
  // สร้าง input ชั่วคราวสำหรับกล้อง
...
function chooseGallery(){
  ...
  var i = parseInt(modal.getAttribute('data-index'));
  closePhotoChoice();
  // สร้าง input ชั่วคราวสำหรับแกลลอรี่
  var inp = document.createElement('input');
  inp.type = 'file';
  inp.accept = 'image/*';
  inp.onchange = function(){
...
function loadSettings(cb){
  db.collection('settings').doc('main').get()
    .then(function(doc){
      ...
      // โหลด mt_settings ตามสาขาด้วย
      var docId = getMtDocId();
      return db.collection('mt_settings').doc(docId).get();
    })
    .then(function(doc){
      if(doc && doc.exists){
        Object.assign(appSettings, doc.data());
      }
      if(cb) cb();
    })
    .catch(function(){if(cb) cb();});
...
function openEditPhotos(recId){
  ...
  _editPhotoData = {
    meter: [ph&&ph.meter?ph.meter[0]:null],
    work: [
      ph&&ph.work?ph.work[0]:null,
      ph&&ph.work?ph.work[1]:null
    ],
    equip: [
      ph&&ph.equip?ph.equip[0]:null,
      ph&&ph.equip?ph.equip[1]:null
    ]
  };
  // สร้าง modal content
  var h = '';
  ...
  groups.forEach(function(g){
    ...
    h += '<b style="color:#4a1660">'+g[1]+'</b>';
    h += '<div style="display:flex;gap:8px;margin-top:6px">';
    for(var i=0;i<g[2];i++){
      var src = _editPhotoData[g[0]][i];
      var imgSrc = src?driveImg(src):'';
      h += '<div style="flex:1;position:relative">';
      if(imgSrc){
        h += '<img src="'+imgSrc+'"';
        h += ' style="width:100%;border-radius:6px;';
        h += 'border:1px solid #ddd">';
      } else {
        h += '<div style="background:#f3f4f6;';
        h += 'height:80px;border-radius:6px;';
        h += 'display:flex;align-items:center;';
        h += 'justify-content:center;color:#9ca3af;';
        h += 'font-size:12px">ไม่มีรูป</div>';
      }
      h += '<div style="margin-top:4px;display:flex;gap:4px">';
      h += '<label class="btn btn-outline btn-sm"';
      h += ' style="cursor:pointer;flex:1;';
      h += 'justify-content:center">';
      h += '<i class="fa fa-camera"></i> เปลี่ยน';
      h += '<input type="file" accept="image/*"';
      h += ' style="display:none"';
      h += ' onchange="editPhotoChange(';
      h += "'"+g[0]+"',"+i+',this)">';
      h += '</label>';
      if(imgSrc){
        h += '<button class="btn btn-danger btn-sm"';
        h += ' onclick="editPhotoRemove(';
        h += "'"+g[0]+"',"+i+')">';
...
function closeEditPhotos(){
  document.getElementById('editPhotosModal').style.display = 'none';
}
  ...
  reader.onload = function(e){
    ...
    img.onload = function(){
      ...
      .then(function(r){
        hideLoading();
        if(r && r.ok){
          _editPhotoData[g][i] = r.url;
          openEditPhotos(_editPhotoRecId);
        } else {
...
function editPhotoRemove(g,i){
  _editPhotoData[g][i] = null;
  openEditPhotos(_editPhotoRecId);
}
function saveEditPhotos(){
  if(!_editPhotoRecId) return;
  showLoading('กำลังบันทึกรูป...');
  db.collection('records').doc(_editPhotoRecId)
    .update({photos:_editPhotoData})
    .then(function(){
      hideLoading();
      closeEditPhotos();
      renderHistory();
...
// -- MT TEMPLATE MANAGEMENT --
function uploadMtTemplate(input){
  var file = input.files[0];
  if(!file) return;
  if(!file.name.endsWith('.docx')){
    alert('กรุณาเลือกไฟล์ .docx');
    return;
  }
  var reader = new FileReader();
  reader.onload = function(e){
    ...
    db.collection('mt_settings').doc(docId)
      .set({
        mtTemplateB64: b64,
        mtTemplateName: file.name,
        mtTemplateDate: Date.now()
      },{merge:true})
      .then(function(){
        hideLoading();
        appSettings.mtTemplateB64 = b64;
        appSettings.mtTemplateName = file.name;
        appSettings.mtTemplateDate = Date.now();
        updateTemplateInfo();
        alert('อัปโหลด template สำเร็จ!');
      })
      .catch(function(){
        hideLoading();
        alert('อัปโหลดไม่สำเร็จ');
...
function resetMtTemplate(){
  ...
  db.collection('mt_settings').doc(docId)
    ...
    .then(function(){
      hideLoading();
      appSettings.mtTemplateB64 = '';
      appSettings.mtTemplateName = '';
      updateTemplateInfo();
      alert('ใช้ template เริ่มต้นแล้ว');
    })
    .catch(function(){
      hideLoading();
      alert('เกิดข้อผิดพลาด');
...
function downloadCurrentTemplate(){
  var b64 = appSettings.mtTemplateB64
    || MT_TEMPLATE_B64;
  var binary = atob(b64);
...
function updateTemplateInfo(){
  var el = document.getElementById('currentTemplateInfo');
  if(!el) return;
  if(appSettings.mtTemplateName){
    var d = appSettings.mtTemplateDate
      ? new Date(appSettings.mtTemplateDate)
          .toLocaleString('th-TH')
      : '';
    el.innerHTML =
      '<span class="badge badge-green">' +
      '<i class="fa fa-file-word"></i> ' +
      appSettings.mtTemplateName + '</span>' +
      '<span style="margin-left:8px;font-size:11px;' +
      'color:#9ca3af">' + d + '</span>';
  } else {
    el.innerHTML =
      '<span class="badge badge-gray">' +
      'ใช้ template เริ่มต้น</span>';
  }
}
  ...
  db.collection('records').get()
    .then(function(snap){
      ...
      snap.forEach(function(doc){
        var d = doc.data();
        d.id = doc.id;
        _orderRecsCache.push(d);
      });
      _orderRecsCache.sort(function(a,b){
        return (b.savedAt||0)-(a.savedAt||0);
      });
      // Staff เห็นแค่สาขาตัวเอง
      var recs = _orderRecsCache;
      if(!isAdmin){
        recs = recs.filter(function(r){
          return r.branch===userBranch;
        });
      }
      renderOrderList(recs);
      renderMtList(recs);
    })
    .catch(function(){
      hideLoading();
...
function renderOrderList(recs){
  ...
  h += '<thead><tr>';
  h += '<th>วันที่</th><th>ผู้ใช้ไฟฟ้า</th>';
  h += '<th>PEA</th><th>สาขา</th>';
  h += '<th>รวมทั้งสิ้น</th>';
  h += '<th>สถานะ</th><th></th>';
  h += '</tr></thead><tbody>';
  if(!recs.length){
    h += '<tr><td colspan="7" style="text-align:center;';
    h += 'color:#9ca3af;padding:20px">';
    h += 'ไม่มีรายการ</td></tr>';
  }
  recs.forEach(function(r){
    var dt = r.savedAt
      ...
      : '';
    var rid = r.id.replace(/'/g,"\\'");
    var st = r.orderStatus||'รอนำเข้าระบบ SAP';
    var stClass = st==='สร้างใบสั่งคิดค่าบริการแล้ว'
    ...
    h += '<td style="text-align:right;font-weight:600">';
    h += fmt(r.grand)+' B</td>';
    h += '<td><select onchange="updateOrderStatus(\'';
    h += rid+'\',this.value)" style="font-size:12px">';
    h += '<option value="รอนำเข้าระบบ SAP"';
    if(st==='รอนำเข้าระบบ SAP') h += ' selected';
    h += '>รอนำเข้าระบบ SAP</option>';
    h += '<option value="สร้างใบสั่งคิดค่าบริการแล้ว"';
    if(st==='สร้างใบสั่งคิดค่าบริการแล้ว') h += ' selected';
    h += '>สร้างใบสั่งแล้ว</option>';
    h += '</select></td>';
    h += '<td><button class="btn btn-outline btn-sm"';
    h += ' onclick="viewRecord(\''+rid+'\')">';
    h += '<i class="fa fa-eye"></i> ดู</button></td>';
    h += '</tr>';
  });
  h += '</tbody></table></div>';
...
function renderMtList(recs){
  ...
  recs.forEach(function(r){
    ...
    h += '<td style="text-align:right;font-weight:600">';
    h += fmt(r.grand)+' B</td>';
    h += '<td><button class="btn btn-primary btn-sm"';
    h += ' onclick="openMtForm(\''+rid+'\')">';
    h += '<i class="fa fa-file-circle-plus"></i>';
    h += ' สร้างใบ มท.</button></td>';
    h += '</tr>';
  });
  h += '</tbody></table></div>';
...
function openMtForm(recId){
  var rec = _orderRecsCache.find(function(x){
    return String(x.id)===String(recId);
  });
  if(!rec){alert('ไม่พบรายการ');return;}
...
function confirmMtForm(){
  ...
  var name = gv('mt_name');
  var address = gv('mt_address');
  var contract = gv('mt_contract');
  var approverVal = gv('mt_approver');
  if(!name||!contract||!approverVal){
    showAlert('mtFormAlert',
      'กรุณากรอกข้อมูลให้ครบ','danger');
    return;
  }
  var parts = approverVal.split('|');
  var approverName = parts[0]||'';
  var approverPosition = parts[1]||'';
  var rec = _orderRecsCache.find(function(x){
    return String(x.id)===String(recId);
  });
  if(!rec){alert('ไม่พบรายการ');return;}
  ...
  db.collection('mt_documents').add(mtData)
    .then(function(){
      hideLoading();
      closeMtForm();
      generateMtPdf(mtData);
    })
    .catch(function(){
      hideLoading();
      alert('เกิดข้อผิดพลาด');
...
function generateMtPdf(data){
  var rec = data.rec;
  var s = data.settings||appSettings||{};
  var now = new Date();
  var thM = [
  ...
  dateStr += thM[now.getMonth()]+' ';
  dateStr += (now.getFullYear()+543);
  var grandText = bahtText(rec.grand||0);
  // ข้อมูลที่จะใส่ใน template
  var templateData = {
    mt_number: s.mtNumber||'',
    mt_branch: s.mtBranch||'',
    mt_addr: s.mtAddr||'',
    mt_road: s.mtRoad||'',
    mt_tambon: s.mtTambon||'',
    mt_amphoe: s.mtAmphoe||'',
    mt_province: s.mtProvince||'',
    mt_zipcode: s.mtZipcode||'',
    date1: dateStr,
    name1: data.name||'',
    address1: data.address||'',
    contract: data.contract||'',
    provider: rec.provider||'',
    work_date: extractDate(rec.datetime||''),
    price1: fmt(rec.part1||0),
    price2: fmt(rec.part2||0),
    price3: fmt(rec.part3||0),
    price4: fmt(rec.vat||0),
    total: fmt(rec.grand||0),
    total_text: grandText,
    approver_name: data.approverName||'',
    approver_pos: data.approverPosition||''
  };
  try {
    // Decode base64 template
    var tpl = appSettings.mtTemplateB64
      || MT_TEMPLATE_B64;
    var binary = atob(tpl);
    var arr = new Uint8Array(binary.length);
    for(var i=0;i<binary.length;i++){
    ...
    var a = document.createElement('a');
    a.href = url;
    var fname = 'มท_' + (rec.pea||'') + '_';
    fname += new Date().toISOString().slice(0,10);
    fname += '.docx';
    a.download = fname;
    document.body.appendChild(a);
    a.click();
    ...
    alert('สร้างไฟล์ มท. ไม่สำเร็จ: '+e.message);
    console.error(e);
  }
}
...
// -- มท. SETTINGS --
function getMtDocId(){
  // ใช้ branch ของ user เป็น key แยก settings
  var branch = currentUser.branch||'default';
  return 'mt_'+branch.replace(/[/\s.]/g,'_');
}
function saveMtSettings(){
  var data = {
    mtBranchDisplay: currentUser.branch||'',
    mtNumber: gv('set_mtNumber'),
    mtBranch: gv('set_mtBranch'),
    mtAddr: gv('set_mtAddr'),
    mtRoad: gv('set_mtRoad'),
    mtTambon: gv('set_mtTambon'),
    mtAmphoe: gv('set_mtAmphoe'),
    mtProvince: gv('set_mtProvince'),
    mtZipcode: gv('set_mtZipcode')
  };
  var approvers = [];
  for(var i=1;i<=5;i++){
    var n = gv('set_apName'+i);
    var p = gv('set_apPos'+i);
    if(n) approvers.push({name:n,position:p});
  }
  ...
  db.collection('mt_settings').doc(docId)
    ...
    .then(function(){
      ...
      showAlert('mtSettingsAlert',
        'บันทึกสำเร็จ ('+currentUser.branch+')','success');
    })
    .catch(function(){
      hideLoading();
      showAlert('mtSettingsAlert','เกิดข้อผิดพลาด','danger');
    });
}
function loadMtSettingsForm(){
  // แสดงสาขาของ user ด้านบน
  var brLabel = document.getElementById('mtBranchLabel');
  if(brLabel){
    brLabel.textContent = currentUser.branch||'';
  }
  // โหลด settings ตามสาขา
  var docId = getMtDocId();
  showLoading('กำลังโหลดตั้งค่า...');
  db.collection('mt_settings').doc(docId).get()
    .then(function(doc){
      hideLoading();
      if(doc.exists){
        appSettings = Object.assign(appSettings, doc.data());
      }
      fillMtForm();
      updateTemplateInfo();
    })
...
function fillMtForm(){
  var s = appSettings;
  var ids = [
    'set_mtNumber','set_mtBranch','set_mtAddr',
    'set_mtRoad','set_mtTambon','set_mtAmphoe',
    'set_mtProvince','set_mtZipcode'
  ];
  var keys = [
    'mtNumber','mtBranch','mtAddr',
    'mtRoad','mtTambon','mtAmphoe',
    'mtProvince','mtZipcode'
  ];
  ...
  for(var j=1;j<=5;j++){
    var a = approvers[j-1]||{};
    var ne = document.getElementById('set_apName'+j);
    var pe = document.getElementById('set_apPos'+j);
    if(ne) ne.value = a.name||'';
    if(pe) pe.value = a.position||'';
  }
}


// -- PROVINCE DROPDOWN สำหรับ ตั้งค่า มท. --
function filterMtProvince(){
  var inp = document.getElementById('set_mtProvince');
  var val = inp.value.toLowerCase();
  var dd = document.getElementById('mtProvinceDD');
  if(!dd) return;
...
function extractDate(dt){
  ...
  // รูปแบบ: '1 มกราคม 2569 14:30 น.' -> '1 มกราคม 2569'
  var m = dt.match(
    /^(\d+\s+\S+\s+\d+)/
  );
  if(m) return m[1];
  // รูปแบบ ISO: '2026-01-15T14:30'
  if(dt.indexOf('T')>-1){
    var d = new Date(dt);
    var thM = [
      'มกราคม','กุมภาพันธ์','มีนาคม','เมษายน',
...
function switchOrderTab(t){
  ...
  document.querySelectorAll('#orderTabs .tab')
    .forEach(function(el,i){
      el.classList.toggle('active',tabs[i]===t);
    });
  ['otOrderList','otMtList','otMtSettings']
    .forEach(function(id){
      document.getElementById(id).classList.remove('active');
    });
  var map = {
    orderList:'otOrderList',
    mtList:'otMtList',
    mtSettings:'otMtSettings'
  };
  document.getElementById(map[t]).classList.add('active');
